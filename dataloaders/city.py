from base import BaseDataSet, BaseDataLoader, BaseDataSet_SubCls
...
class CityDataset_SubCls(BaseDataSet_SubCls):
    def __init__(self, **kwargs):
        self.num_classes = 19
        self.datalist = kwargs.pop("datalist")
        self.palette = pallete.get_voc_pallete(self.num_classes)
        ...
        super(CityDataset_SubCls, self).__init__(**kwargs)
    ...
    def _load_data(self, index):
        ...
        label_path_subcls = os.path.join(self.label_subcls, self.labels[index][1:].split('/')[-1].replace('_gtFine_labelTrainIds.png','_leftImg8bit.png'))
        label_subcls = np.asarray(Image.open(label_path_subcls), dtype=np.int32)
        return image, label, label_subcls, image_id

class City_SubCls(BaseDataLoader):
    def __init__(self, kwargs):
        self.MEAN = [0.485, 0.456, 0.406]
        self.STD = [0.229, 0.224, 0.225]
        self.batch_size = kwargs.pop('batch_size')
        ...
        self.dataset = CityDataset_SubCls(**kwargs)
        shuffle = False
        dist_sampler = torch.utils.data.distributed.DistributedSampler(self.dataset, shuffle=sampler_shuffle)

        super(City_SubCls, self).__init__(self.dataset, self.batch_size, shuffle, num_workers, val_split=None,
                                   dist_sampler=dist_sampler)

class PairCityDataset_StrongWeak(BaseDataSet):
    def __init__(self, **kwargs):
        self.num_classes = 19
        ...
        self.palette = pallete.get_voc_pallete(self.num_classes)
        super(PairCityDataset_StrongWeak, self).__init__(**kwargs)
...
class PairCity_StrongWeak(BaseDataLoader):
    def __init__(self, kwargs):
        self.MEAN = [0.485, 0.456, 0.406]
        self.STD = [0.229, 0.224, 0.225]
        self.batch_size = kwargs.pop('batch_size')
        ...
        sampler_shuffle = kwargs.pop('shuffle')
        num_workers = kwargs.pop('num_workers')
        self.dataset = PairCityDataset_StrongWeak(**kwargs)
        shuffle = False
        dist_sampler = torch.utils.data.distributed.DistributedSampler(self.dataset, shuffle=sampler_shuffle)

        super(PairCity_StrongWeak, self).__init__(self.dataset, self.batch_size, shuffle, num_workers, val_split=None,
                                                  dist_sampler=dist_sampler)