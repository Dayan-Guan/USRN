...
def CE_ohem_loss(input_logits, target_targets, ignore_index, temperature=1):
    batch_size = 8
    world_size = 1
    pixel_num = 50000 * batch_size // world_size
    criterion = ProbOhemCrossEntropy2d(ignore_label=ignore_index, thresh=0.7,
                                       min_kept=pixel_num, use_weight=False)
    return criterion(input_logits, target_targets)
